describe('Equasion', () => {
  describe('#result', () => {
    context('for valid equasion', () => {
      it('returns result', () => {
        var equasion = new Equasion(new Tokenizer().fromString('301 + 4 * (20 - 1)'));

        chai.expect(equasion.result).to.equal(377);
      });

      it('returns result', () => {
        var equasion = new Equasion(new Tokenizer().fromString('3 * (2 + 5)'));

        chai.expect(equasion.result).to.equal(21);
      });

      it('returns result', () => {
        var equasion = new Equasion(new Tokenizer().fromString('((((1*(2+3))-3)+4)*5)'));

        chai.expect(equasion.result).to.equal(30);
      });

      it('returns result', () => {
        var equasion = new Equasion(new Tokenizer().fromString('(300+23)*(43-21)/(84+7)'));

        chai.expect(equasion.result).to.equal(78.08791208791209);
      });

      it('returns result', () => {
        var equasion = new Equasion(new Tokenizer().fromString('(4+8)*(6-5)/((3-2)*(2+2))'));

        chai.expect(equasion.result).to.equal(3);
      });

      it('returns result', () => {
        var equasion = new Equasion(new Tokenizer().fromString('( (10+20) + 30 ) * 20-8/4'));
    ...
    context('for invalid equasion', () => {
      it('throws exception', () => {
        var equasion = new Equasion(new Tokenizer().fromString('- -'));

        chai.expect(() => equasion.result).to.throw(Error, 'Invalid syntax!');
      });

      it('throws exception', () => {
        var equasion = new Equasion(new Tokenizer().fromString('-1'));
  ...
  describe('#postfix', () => {
    context('for valid syntax', () => {
      ...
      it('returns postfix', () => {
        var equasion = new Equasion(new Tokenizer().fromString('d5'));
